...
try: import imageio
    ...
    msg = 'WARNING in plottools: could not import package "imageio"' 
    msg += ' (this is only used to create gif animations, so you can proceed without for regular plots).'
    print(msg)
    
# try importing latex
# todo: switch from latex to mpl internal formatting
try:
    from matplotlib import rc
    rc('text', usetex=True)
    plot_utils_latex_formatting = True
except:
    msg = 'WARNING: could not set LaTEX rendering for matplotlib.'
    msg += ' Any TEX commands in figure labels might not work as expected.'
    print(msg)
...
#     print()
    ...
    # make the plot
    ax.imshow(hist, aspect=aspect, interpolation='none', norm=norm, cmap=cmap,
...
def plot_hists_2d_gif( hists, 
    ...
    for i in range(nhists):
        ...
        fig.savefig(filename, facecolor='white', transparent=False, bbox_inches='tight', dpi=dpi)
        plt.close()
    # convert to gif
    if mode=='imageio':
